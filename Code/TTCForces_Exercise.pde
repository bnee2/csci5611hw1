...
void setup(){
  ...
  agentPos[3] = new Vec2(120,300);
  agentPos[4] = new Vec2(0,200);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(700,350);
  goalPos[4] = new Vec2(500,200);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  ...
  Vec2 relativeVelocity = vel2.minus(vel1);
  float ttc = rayCircleIntersectTime(pos1, combinedRadius, pos2, relativeVelocity);
  return ttc;
}
...
Vec2 computeAgentForces(int id){
  ...
  /*********************************** Goal Force ***********************************/
  // Goal position is position of goal with corresponding id
  // Goal direction is difference between goal position and current position
  // Goal velocity is goal speed in goal direction
  Vec2 goalPosition = goalPos[id];
  Vec2 goalVelocity = goalPosition.minus(agentPos[id]);
  goalVelocity.setToLength(goalSpeed);
  acc.add(goalVelocity.minus(agentVel[id]).times(k_goal));

  /*********************************** Avoidance Force ***********************************/
  // Only compute avoidance force if agent hasn't reached goal
  if (agentPos[id].minus(goalPos[id]).length() > agentRad) {
    for (int i = 0; i < numAgents; i++) {
      if (i == id) { // Don't compute avoidance force with self
        continue;
      }

      float timeToCollision = computeTTC(agentPos[id], agentVel[id], agentRad, agentPos[i], agentVel[i], agentRad);
      if (timeToCollision == -1) { // Avoidance force 0 for non-colliding agents
        continue;
      }

      Vec2 thisAgentPositionAtCollision = agentPos[id].plus(agentVel[id].times(timeToCollision));
      Vec2 otherAgentPositionAtCollision = agentPos[i].plus(agentVel[i].times(timeToCollision));
      Vec2 relativeVector = thisAgentPositionAtCollision.minus(otherAgentPositionAtCollision).normalized();

      acc.add(relativeVector.times(k_avoid * (1 / timeToCollision)));
    }
  }

  //println(computeTTC(agentPos[0], agentVel[0], agentRad, agentPos[1], agentVel[1], agentRad));
  return acc;
  //return new Vec2(0,0);
}
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    // Agents drawn smaller than their computational representations
    circle(agentPos[i].x, agentPos[i].y, agentRad*1.75);
  }
}
