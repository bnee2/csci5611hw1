...
TODO:
  1. Add a function so that when you press the 'r' key, the obstacles move to
     ...
     [Hint: Look at setup() to see how I place obstacles, build the PRM, and run the BFS]
        - Easy, just call the setup() function again
  ...
  2. Currently edge connections of any length are allowed. Update the connectNeighbors()
     ...
     for this? Why do you think people sometimes put a maximum length on PRM edges?
        - Limiting the PRM edge length might make the it a little bit faster to finalize the PRM
          because a majority of very long PRM edges will be blocked by an obstacle on a sufficiently
          dense field, so they're not even worth checking usually. It will also significantly reduce
          the number of edges to check overall, even if edges did connect all the way across.
  ...
  3. The function, closestNode() is supposed to return the ID of the PRM node
     ...
     the mouse lets you select the red goal node.
        - Added loop over all nodes to find closest node to point
  ...
  5. For the PRM to be collision free wrt the box we need to make sure no nodes are
    ...
    be collision free with all of the circles and the box.
        - Done, needed to fix box boundary checks though because I made an incorrect assumption about
          the orientation of the coordinate grid
...
// Update the PRM to account for box moving
void updatePRM() {
  int nodesChanged = 0;
  Vec2 randPos = new Vec2(0,0);

  // Make sure no nodes are in box, regenerate nodes that are
  for (int i = 0; i < numNodes; i++) {
    boolean insideAnyCircle = false;
    boolean insideTheBox = pointInBox(boxTopLeft, boxW, boxH, nodePos[i]);
    boolean startedInsideTheBox = insideTheBox;

    // Find a new position not in the box or any circles
    while (insideAnyCircle || insideTheBox){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circlePos,circleRad,randPos);
      insideTheBox = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }

    // Update this node's position if it was moved
    if (startedInsideTheBox) {
      nodePos[i] = randPos;
      nodesChanged += 1;
    }
  }

  // Update neighbors and path if necessary
  if (nodesChanged > 0) {
    connectNeighbors();
    runBFS(closestNode(startPos),closestNode(goalPos));
  }
...
int closestNode(Vec2 point){
  int nodeId = -1;
  float minDistance = startPos.distanceTo(goalPos);
  for (int i = 0; i < numNodes; i++) {
    float distance = point.distanceTo(nodePos[i]);
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  float topBoundary = boxTopLeft.y;
  float bottomBoundary = boxTopLeft.y + boxH;
  float leftBoundary = boxTopLeft.x;
  float rightBoundary =  boxTopLeft.x + boxW;
  if (pointPos.x >= leftBoundary &&
      pointPos.x <= rightBoundary &&
      pointPos.y >= topBoundary &&
      pointPos.y <= bottomBoundary) {
    return true;
  }
  return false;
}
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideTheBox = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (insideAnyCircle || insideTheBox){
      randPos = new Vec2(random(width),random(height));
