...
void update(float dt){
  ...
  if (mousePos.distanceTo(pos) < 10) { return; } // Don't update if nearly centered on mouse to prevent jitter
  // if (speed*dp > dir.length()) { vel = new Vec2(0,0); pos = mousePos; } else { vel = dir.normalized(); }
  Vec2 dir = mousePos.minus(pos); // Vector difference is pointer from A to B
  if (dir.length() > 0) dir.normalize();
  vel = dir.times(speed*dt); // Multiply speed by 1/framerate to transform into pixels per second
  pos.add(vel);
}
...
void draw(){
  ...
  stroke(0,0,0); // Changed to black outline
  fill(255,0,0); // Changed to red ball
  circle(pos.x, pos.y,r*2);  // Answer: because the "extent" is the width and height value (the diameter)
}
